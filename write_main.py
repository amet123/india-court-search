...
async def _keyword_search(pool, query, year=None, disposal=None, limit=10):
    ...
    if year:
        params.append(year)
        conditions.append(f"c.year = \${len(params)}")
    if disposal:
        params.append(f"%{disposal}%")
        conditions.append(f"c.disposal_nature ILIKE \${len(params)}")
    params.append(limit)
    where = " AND ".join(conditions)
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            f"SELECT c.case_id, c.year, c.title, c.petitioner, c.respondent, c.date_of_judgment::TEXT, c.disposal_nature, c.bench, c.citation, c.pdf_url, ch.chunk_text, ts_rank_cd(ch.ts_vector, plainto_tsquery('english', $1)) AS score FROM chunks ch JOIN cases c ON c.case_id = ch.case_id WHERE {where} ORDER BY score DESC LIMIT \${len(params)}",
            *params,
        )
...
async def _generate_answer(request, query, results, model):
    context = "\\n\\n".join(f"[{r.get('title','')} ({r.get('year','')})]\n{r['chunk_text']}" for r in results[:6])
    try:
        resp = await request.app.state.anthropic.messages.create(model=model, max_tokens=1500, system="You are an expert Indian legal research assistant.", messages=[{"role": "user", "content": f"Question: {query}\n\nContext: {context}"}])
